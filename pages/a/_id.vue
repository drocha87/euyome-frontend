<template>
  <v-row class="mx-auto fill-height" justify="center" style="max-width: 900px">
    <v-col class="text-center px-8" align-self="center">
      ...
      <div v-if="agency.bio" class="text-justify mt-6" style="color: #3d405b">{{ bio }}</div>
    </v-col>
    <v-col>
      <v-card class="mx-auto" flat>
        ...
        <v-card-text>
          ...
          <div
            v-if="searchLen > 1"
            class="text-right text-caption my-1 mr-3"
          >{{ searchLen }} perfis encontrados</div>
          <div class="list-profiles mt-2">
            <client-only>
              ...
              >
                <template v-slot:no-data>Ainda não tem nenhum perfil criado!</template>
                <template #item="{ item }">
